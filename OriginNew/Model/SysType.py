...
class SysType(Base):
    ...
    def get_command(self, control_type: ControlType = ControlType.QUERY, condition: str = None) -> str or None:
        ...
        if control_type == ControlType.INSERT:
            command = "INSERT INTO {} (P_ID, `Name`, Frozen, Description) VALUES ({}, '{}', {}, '{}')"\
                .format(self.TableName, self.PID, self.Name, self.Frozen,
                        '' if self.Description is None else self.Description)
        elif control_type == ControlType.UPDATE:
            command = "UPDATE {} SET P_ID={}, `Name`='{}', Frozen={}, Description='{}' WHERE ID={}".format(
                self.TableName, self.PID, self.Name, self.Frozen, '' if self.Description is None else
